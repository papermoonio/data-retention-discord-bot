...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'ping') {
        ...
        while (loopMessageDeletion) {
            ...
            const youngerTimestampThreshold = dateThreshold.valueOf();
            const olderTimestampThreshold = 0
            ...
            for (const m in oldMsgs) {
                if(!loopMessageDeletion) {
                    await interaction.channel?.send(`Deletion process halted, but deletion round is unfinished.`);
                    return;
                }
                await interaction.channel?.messages.delete(oldMsgs[m]);
            }
            await interaction.channel?.send(`Deletion process finished. Will repeat in 5 minutes.`);
